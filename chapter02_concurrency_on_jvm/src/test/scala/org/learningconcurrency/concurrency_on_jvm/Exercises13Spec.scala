...
class Exercises13Spec extends BaseSpec{
  ...
  "concurrently adding and then swapping 100000 random values to ConcurrentBiMap" should "" +
    "keep the map size the same" in {
    val target = new ConcurrentBiMap[Long, Long]
    val workers = PriorityTaskPool_Ex9(4)
    (1 to 100000).foreach {i => workers.asynchronous(i){() => target.put(Random.nextLong(), Random.nextLong())}}
    Thread.sleep(1000)
