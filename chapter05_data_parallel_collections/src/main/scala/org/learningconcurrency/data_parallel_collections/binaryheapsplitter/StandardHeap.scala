...
import org.learningconcurrency.data_parallel_collections.binaryheapsplitter.BinaryHeapSplitter.IsOrdered
...
abstract sealed class Heap[+A : IsOrdered] {
   ...
   * Space - O(log n)
   */
  def insert[B >: A : IsOrdered](x: B): Heap[B] =
   ...
   */
  def remove: Heap[A] = {
    def floatLeft[A : IsOrdered](x: A, l: Heap[A], r: Heap[A]): Heap[A] = l match {
      case Branch(y, lt, rt, _, _) => Heap.make(y, Heap.make(x, lt, rt), r)
      case _ => Heap.make(x, l, r)
    }

    def floatRight[A : IsOrdered](x: A, l: Heap[A], r: Heap[A]): Heap[A] = r match {
...
case class Branch[A : IsOrdered](min: A, left: Heap[A], right: Heap[A], size: Int, height: Int) extends Heap[A] {
...
object Heap {
   ...
   */
  def make[A : IsOrdered](x: A, l: Heap[A] = Leaf, r: Heap[A] = Leaf): Heap[A] =
   ...
   * Space - O(log n)
   */
  def fromSortedArray[A : IsOrdered](a: Array[A]): Heap[A] = {
   ...
   * Space - O(log n)
   */
  def fromArray[A : IsOrdered](a: Array[A]): Heap[A] = {
    def loop(i: Int): Heap[A] =
   ...
   * Space - O(1)
   */
  private[Heap] def bubbleUp[A : IsOrdered](x: A, l: Heap[A], r: Heap[A]): Heap[A] = (l, r) match {
   ...
   * Space - O(log n)
   */
  private[Heap] def bubbleDown[A : IsOrdered](x: A, l: Heap[A], r: Heap[A]): Heap[A] = (l, r) match {
